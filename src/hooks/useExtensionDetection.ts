...
export const useExtensionDetection = (): ExtensionDetectionResult => {
      ...
      if (typeof window !== 'undefined') {
        try {
          ...
          const avastDetected =
            ...
            console.error.toString().includes('AT-SDK') ||
          ...
          if (avastDetected) {
            extensions.push({
              ...
              detected: true,
              interference: 'high',
              description: 'Known to interfere with WebGL contexts and 3D graphics'
            });
          }
          ...
          const hasExtensionErrors = extensionErrors.some(error => {
            ...
            return console.error.toString().includes(error) ||
          ...
          if (hasExtensionErrors && extensions.length === 0) {
            extensions.push({
              ...
              detected: true,
              interference: 'medium',
              description: 'Detected extension-related interference with page functionality'
            });
          }

          // Check for Transparent Zen or similar themes
          const themeExtensionDetected =
            document.documentElement.style.getPropertyValue('--zen-logo-path') ||
          ...
          if (themeExtensionDetected) {
            extensions.push({
              ...
              detected: true,
              interference: 'medium',
              description: 'Browser theme that modifies page appearance and may affect functionality'
            });
          }
  ...
  const problematicExtensions = detectedExtensions.filter(ext =>
    ext.interference === 'high' || ext.interference === 'medium'
  );
